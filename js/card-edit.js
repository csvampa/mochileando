...
createApp({
  data() {
    return {
      ...
      urlImg: "",
      description: "",
      url: "https://iroldan.pythonanywhere.com/cards/" + id,
    };
  },
  methods: {
    fetchData(url) {
      ...
      En el primer then, se convierte la respuesta en formato JSON mediante response.json(). Luego, en el segundo then, se accede a los datos obtenidos y se asignan a las propiedades de la instancia de Vue, como id, city, country, urlImg y description.
    ...
    modificar() {
      /* Este código define la función modificar(). En esta función, se crea un objeto card que contiene las propiedades city, description, urlImg e country obtenidas de la instancia de Vue. Luego se define un objeto options que especifica las opciones para la solicitud HTTP.
